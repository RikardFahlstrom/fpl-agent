...
- **Read scoring weights from `game_config`.** Never hardcode them. FPL changes them
  between seasons; `defensive_contribution` is new this year. See `scoring.py`.
...
- **Never grade a gameweek that has not finished.** Absence of an actual is not a zero
  until the fixtures are played. See `settle.gameweek_is_finished`.
...
| Fact | Where |
| --- | --- |
| Price change rule: Predicted Progress > 100% is "Very Likely"; `likelihood` is a derived band of the same number | `pricing.py` |
| Defensive-contribution thresholds (DEF >= 10, MID >= 12) are not published; derived from 1236 scored appearances | `scoring.py` |
| `/me/` carries no league membership - leagues are on `entry/{id}/` | `state.get_user_leagues` |
| `league_type` `x` is a private league, `s` is global and unusable ("Overall" has ~9.9M entries) | `rivals.py` |
| Per-90 rates from tiny samples must be shrunk toward a prior | `projection.shrink` |
| The sell-on fee returns only half of any profit, so budget grows slower than the market | `pricing.py` |
