...
| The account service rotates the refresh token on every exchange, so two concurrent refreshes leave one caller holding a dead credential | `api/headless_auth.save_refreshed_session` |
| Rival picks for a round exist only after its deadline, so ownership is fresh when the rivals gameweek is not behind the last *finished* one; stale ownership is not shown anywhere rather than shown with a caveat, and every channel renders a move through one function | `engine/warehouse.OwnershipSource`, `recommend.move_lines` |
| The current gameweek is the only one with predicted lineups, so it beats every later week almost by construction; a chip verdict therefore needs two gates - the chip's bar *and* best-of-window - or every chip is played the first week it is valued | `engine/chips.CHIP_BARS`, `chips.decide` |
| The club limit is FPL's `game_config` rule `squad_team_limit`, read once into the held squad; the chip rebuilds used a hardcoded 3 while transfers read the published one | `engine/held.team_limit` |
| Who starts (positions 1-11), what a bench slot is worth (0.15), who is buyable (`a`, `d`) and what the chip payload says are each stated once, in the held-squad module; transfers, chips, the rebuilt squad, the brief and rivals read them there | `engine/held` |
...
| The deadline is FPL's own `deadline_time`, stored per capture for the gameweek it targets; without one (captures before the column existed, gameweeks beyond the target, warehouses a read-only reader has not seen migrated) it is derived as 90 minutes before the first kickoff, played or not - which a postponed opening fixture would move while the real deadline stays | `engine/warehouse.deadline`, `storage.create_snapshot` |
