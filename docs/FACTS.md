...
| The deadline is 90 minutes before the target gameweek's first kickoff, played or not, derived from stored fixtures because the warehouse does not keep `deadline_time`; a postponed opening fixture moves the kickoff but not the real deadline | `engine/warehouse.deadline`, `warehouse.target` |
